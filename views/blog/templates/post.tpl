{template BlogPost BlogPost}
	<div class="blog-post">
		...
		<p class="blog-post-meta">
			{date_format $BlogPost->Created "%B %e, %Y"} by <a href="/about">Brooke</a>{if \technexus\App::is_loggedin()}{/if}<span class="float-right">{if $BlogPost->Status=='Draft'}<span class="badge badge-secondary" title="Not visible to guests.">Draft</span>{/if}<a class="btn-sm btn" href="/admin/posts/{$BlogPost->ID}/"><i class="fas fa-edit"></i> Edit</a></span>
			{if $BlogPost->Tags}
				<br>
